...
class MySqlDump
{
	...
	public function start($filename = '')
	{
		...
		if ($this->dumpSettings['databases']) {
			$this
				...
				->write($this
					->typeAdapter
					->getDatabaseHeader($this->dbName));
			if ($this->dumpSettings['add-drop-database']) {
				$this
					->compressManager
					->write($this
						->typeAdapter
						->add_drop_database($this->dbName));
			}
		}
	...
	private function getDatabaseStructureTables()
	{
		...
		if (empty($this->dumpSettings['include-tables'])) {
			...
			foreach ($this
				...
				->query($this
					->typeAdapter
					->show_tables($this->dbName)) as $row) {
	...
	private function getDatabaseStructureTriggers()
	{
		...
		if (false === $this->dumpSettings['skip-triggers']) {
			foreach ($this
				...
				->query($this
					->typeAdapter
					->show_triggers($this->dbName)) as $row) {
	...
	private function getDatabaseStructureProcedures()
	{
		...
		if ($this->dumpSettings['routines']) {
			foreach ($this
				...
				->query($this
					->typeAdapter
					->show_procedures($this->dbName)) as $row) {
	...
	private function getDatabaseStructureFunctions()
	{
		...
		if ($this->dumpSettings['routines']) {
			foreach ($this
				...
				->query($this
					->typeAdapter
					->show_functions($this->dbName)) as $row) {
	...
	private function getDatabaseStructureEvents()
	{
		...
		if ($this->dumpSettings['events']) {
			foreach ($this
				...
				->query($this
					->typeAdapter
					->show_events($this->dbName)) as $row) {
	...
	private function getViewStructureTable($viewName)
	{
		...
		foreach ($this
			...
			$this
				->compressManager
				->write($this->createStandInTable($viewName));
			break;
		}
	}
	...
	private function getFunctionStructure($functionName)
	{
		...
		$stmt = $this
			->typeAdapter
			->show_create_function($functionName);
		foreach ($this
			->dbHandler
			->query($stmt) as $r) {
			$this
	...
	private function listValues($tableName)
	{
		...
		foreach ($resultSet as $row) {
			...
			if ($onlyOnce || !$this->dumpSettings['extended-insert']) {
				if ($this->dumpSettings['complete-insert']) {
					$lineSize += $this
						->compressManager
						->write("INSERT$ignore INTO `$tableName` (" . implode(", ", $colNames) . ") VALUES (" . implode(",", $vals) . ")");
				} else {
					$lineSize += $this
						->compressManager
						->write("INSERT$ignore INTO `$tableName` VALUES (" . implode(",", $vals) . ")");
				}
				$onlyOnce = false;
			} else {
				$lineSize += $this
		...
		if ($this->infoCallable) {
			call_user_func($this->infoCallable, 'table', array(
				'name' => $tableName,
				'rowCount' => $count
			));
		}
	...
	public function prepareListValues($tableName)
	{
		if (!$this->dumpSettings['skip-comments']) {
			$this
				->compressManager
				->write("--" . PHP_EOL . "-- Dumping data for table `$tableName`" . PHP_EOL . "--" . PHP_EOL . PHP_EOL);
		}
		...
		if ($this->dumpSettings['add-locks']) {
			$this
				...
				->write($this
					->typeAdapter
					->start_add_lock_table($tableName));
		}
		...
		if ($this->dumpSettings['disable-keys']) {
			$this
				...
				->write($this
					->typeAdapter
					->start_add_disable_keys($tableName));
		}
	...
	public function endListValues($tableName, $count = 0)
	{
		if ($this->dumpSettings['disable-keys']) {
			$this
				...
				->write($this
					->typeAdapter
					->end_add_disable_keys($tableName));
		}
		...
		if (!$this->dumpSettings['skip-comments']) {
			$this
				->compressManager
				->write("-- Dumped table `" . $tableName . "` with $count row(s)" . PHP_EOL . '--' . PHP_EOL . PHP_EOL);
		}
...
class CompressGzipstream extends CompressManagerFactory
{
	...
	public function open($filename)
	{
		...
		$this->compressContext = deflate_init(ZLIB_ENCODING_GZIP, array(
			'level' => 9
		));
		return true;
	}
...
class TypeAdapterMysql extends TypeAdapterFactory
{
	...
	public function lock_table()
	{
		...
		return $this
			->dbHandler
			->exec("LOCK TABLES `${args[0]}` READ LOCAL");
	}
